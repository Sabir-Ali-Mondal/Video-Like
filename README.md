...
*   **Auto-Subtitles:** As the video renders, the engine tracks the text-to-speech timing and automatically generates a perfectly synced `.srt` subtitle file.
*   **Zero Dependencies:** No servers, no build steps, no installations. Everything from the UI to the rendering engine is contained in one file.

---

### 📘 The Complete Technical Guide & Blueprint
...
3. Use the integrated AI Architect to generate a script, or open the JSON Editor to write your own.
4. Press Play to watch your code come to life.
5. [ If PC ] Open the menu and click "Render HD Video" to export your final `.webm` and `.srt` files.
6. [ If Mobile ] Screen record with media enabled. Crop in Google Photos.
